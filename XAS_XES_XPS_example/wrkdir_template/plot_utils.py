...
def process_quanty_spectra_xps(data, x_shift = 0, left = -100, right = 100):
    data[0] += x_shift
...
def make_QuantyPlots_XASXESXPS_simple(data_path, plot_params_dict):
    """
    Automatically create summary figure of XAS XES and XPS results,
    all isotropic, no dependence on polarization
    """
    ...
    data_XAS = CF.alignpeak(extract_quanty_spectra(data_path / "XAS.dat"))
    data_XES = CF.alignpeak(extract_quanty_spectra(data_path / "XES.dat"))
    data_XPS = CF.alignpeak(extract_quanty_spectra(data_path / "XPS.dat"))

    data_XAS = process_quanty_spectra_xas(data_XAS, x_shift = 0, left = LEFT, right = RIGHT)
    data_XES = process_quanty_spectra_xes(data_XES, x_shift = 0, left = LEFT, right = RIGHT)
    data_XPS = process_quanty_spectra_xps(data_XPS, x_shift = 0, left = LEFT, right = RIGHT)

    ax[0].plot(data_XAS[0], data_XAS[1], label = "XAS, Iso", **plot_params_dict["iso"])
    ax[1].plot(data_XES[0], data_XES[1], label = "XES, Iso", **plot_params_dict["iso"])
    ax[2].plot(data_XPS[0], data_XPS[1], label = "XPS, Iso", **plot_params_dict["iso"])

    ### Aesthetics
    # Axes Labels, Font Size, and Title
    title = CF.Convert_CompName_ToLaTeX(plot_params_dict["NAME"])
    ax[0].set_title(title + " L23 XAS")
    ax[1].set_title(title + " Ka XES")
    ax[2].set_title(title + " 2p XPS")
    for a in ax.flatten():
