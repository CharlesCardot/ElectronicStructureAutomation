...
replacement_dict = {
    ...
    "DFT_PATH": DFT_PATH,
    "TM_ELEMENT": material_input["TM"],
    "TM_NUMBER": TM_NUMBER,
    "TM_FPLO_XYZ": TM_FPLO_XYZ,
    "NUMBER_OF_D": str(material_input["nd"]),
    "TM_CLUSTER_RADIUS": cluster_radius
}
...
replacement_dict["TM_ZETA_2P"] = TM_ZETA_2P 
replacement_dict["TM_ZETA_3P"] = TM_ZETA_3P
replacement_dict["TM_ZETA_3D"] = TM_ZETA_3D


# For calculating polarized spectra (ex: under external magnetic field)
if "TM_MAG_DIR" in material_input and "TM_H_MAG" in material_input:
    TM_MAG_DIR = np.asarray(material_input["TM_MAG_DIR"])
    TM_MAG_DIR = np.round(TM_MAG_DIR / np.sum(TM_MAG_DIR), 2) # normalize
    ...
    TM_H_MAG = "0"
    replacement_dict["TM_MAG_DIR"] = TM_MAG_DIR 
    replacement_dict["TM_H_MAG"] = TM_H_MAG

if "DDDD" in material_input and "UUUU" in material_input:
...
for key, line in enumerate(lines):
    ...
    for dict_key, val in replacement_dict.items():
        line = line.replace(dict_key, val)
