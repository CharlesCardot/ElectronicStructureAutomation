...
def extract_feff_fermienergy(filename):
    with open(filename, "r") as f:
        lines = f.readlines()
    for line in lines:
        if line.startswith("# # Mu="):
            line = line.split()[2][3:13]
            return float(line)

def extract_feff_ldos(filename):
    with open(filename, "r") as f:
        lines = f.readlines()
    fermienergy = float(lines[0].split()[-1])
    data = np.loadtxt(filename, comments="#").T
    sDOS = np.asarray([data[0], data[1]])
    pDOS = np.asarray([data[0], data[2]])
    dDOS = np.asarray([data[0], data[3]])
    fDOS = np.asarray([data[0], data[4]])
    return fermienergy, sDOS, pDOS, dDOS, fDOS

def process_quanty_spectra(data, x_shift = 0, left = -100, right = 100):
    data[0] += x_shift
...
def combine_spectra(spectra, weights = None):

    if not weights:
        weights = [1 for i in range(len(spectra))]
    
    num_points = 1000 # I set this arbitrarily

    # Find the common range of x values
    min_x = min([min(spectrum[0]) for spectrum in spectra])
    max_x = max([max(spectrum[0]) for spectrum in spectra])

    # Generate equally spaced x values
    x_values = np.linspace(min_x, max_x, num_points)

    # Initialize combined spectrum
    combined_spectrum = np.zeros((2, num_points))

    # Interpolate y values for each spectrum and combine
    for key, spectrum in enumerate(spectra):
        y_values = np.interp(x_values, spectrum[0], spectrum[1])
        combined_spectrum[1] += y_values * weights[key]

    # Normalize combined spectrum
    combined_spectrum[1] /= len(spectra)

    # Assign x values
    combined_spectrum[0] = x_values
...
def make_QuantyPlots(data_path, plot_params_dict):
    """
    ...
    data_z_lin = process_quanty_spectra(extract_quanty_spectra(data_path / "XES_zpol.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT)
    data_y_lin = process_quanty_spectra(extract_quanty_spectra(data_path / "XES_ypol.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT)
    data_x_lin = process_quanty_spectra(extract_quanty_spectra(data_path / "XES_xpol.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT)
    
    data_z_rig = process_quanty_spectra(extract_quanty_spectra(data_path / "XES_rpol_zdir.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT)
    data_z_lef = process_quanty_spectra(extract_quanty_spectra(data_path / "XES_lpol_zdir.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT)
    data_y_rig = process_quanty_spectra(extract_quanty_spectra(data_path / "XES_rpol_ydir.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT)
    data_y_lef = process_quanty_spectra(extract_quanty_spectra(data_path / "XES_lpol_ydir.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT)
    data_x_rig = process_quanty_spectra(extract_quanty_spectra(data_path / "XES_rpol_xdir.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT)
    data_x_lef = process_quanty_spectra(extract_quanty_spectra(data_path / "XES_lpol_xdir.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT)
    
    data_iso = process_quanty_spectra(data_iso, x_shift = X_SHIFT, left = LEFT, right = RIGHT)
...
def make_FEFFPlots(data_paths, plot_params_dict):
    """
    Automatically create summary figure of VtC-XES results,
    assumes that isotropic and all linear polarizations have
    been calculated.

    Update: Have added the ability to average over spectra from
    multiple folders. This is necessary for averaging over multiple
    Ligand wyckoff positions, for example.
    """
    ...
    weights = [] # Used for weighting the spectra by symmetry multiplicty
    for data_path in data_paths:
        data_path = Path(data_path)
        weights.append(plot_params_dict["wyckoff_weights"][data_path.name.split("_")[-1]])
        
        # Calculate x_shift
        X_SHIFT = -1 * extract_feff_fermienergy(data_path / "isotropic" / "xmu.dat")
        
        ### Read in and process feff spectra
        # Just Dipole
        data_iso_jd.append(process_feff_spectra(extract_feff_spectra(data_path / "isotropic" / "xmu.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT))
        data_z_lin_jd.append(process_feff_spectra(extract_feff_spectra(data_path / "z_polarization" / "xmu.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT)) 
        data_y_lin_jd.append(process_feff_spectra(extract_feff_spectra(data_path / "y_polarization" / "xmu.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT))
        data_x_lin_jd.append(process_feff_spectra(extract_feff_spectra(data_path / "x_polarization" / "xmu.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT))

        data_z_dir_jd.append(avg_two_spectra(data_x_lin_jd[-1], data_y_lin_jd[-1])) 
        data_y_dir_jd.append(avg_two_spectra(data_z_lin_jd[-1], data_x_lin_jd[-1]))
        data_x_dir_jd.append(avg_two_spectra(data_y_lin_jd[-1], data_z_lin_jd[-1]))

        # Dipole + Quadrupole
        data_iso_dq.append(process_feff_spectra(extract_feff_spectra(data_path / "isotropic_withquad" / "xmu.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT))
        data_z_dir_dq.append(process_feff_spectra(extract_feff_spectra(data_path / "z_direction_withquad" / "xmu.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT))  
        data_y_dir_dq.append(process_feff_spectra(extract_feff_spectra(data_path / "y_direction_withquad" / "xmu.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT))
        data_x_dir_dq.append(process_feff_spectra(extract_feff_spectra(data_path / "x_direction_withquad" / "xmu.dat"), x_shift = X_SHIFT, left = LEFT, right = RIGHT))

        # Just Quadrupole
        data_iso_jq.append(np.asarray([data_iso_dq[-1][0], data_iso_dq[-1][1] - data_iso_jd[-1][1]]))
        data_z_dir_jq.append(np.asarray([data_z_dir_dq[-1][0], data_z_dir_dq[-1][1] - data_z_dir_jd[-1][1]])) 
        data_y_dir_jq.append(np.asarray([data_y_dir_dq[-1][0], data_y_dir_dq[-1][1] - data_y_dir_jd[-1][1]]))
        data_x_dir_jq.append(np.asarray([data_x_dir_dq[-1][0], data_x_dir_dq[-1][1] - data_x_dir_jd[-1][1]]))

    # Just Dipole
    data_iso_jd = combine_spectra(data_iso_jd, weights)
    data_z_lin_jd = combine_spectra(data_z_lin_jd, weights) 
    data_y_lin_jd = combine_spectra(data_y_lin_jd, weights)
    data_x_lin_jd = combine_spectra(data_x_lin_jd, weights)

    data_z_dir_jd = combine_spectra(data_z_dir_jd, weights)
    data_y_dir_jd = combine_spectra(data_y_dir_jd, weights)    
    data_x_dir_jd = combine_spectra(data_x_dir_jd, weights)

    # Dipole + Quadrupole
    data_iso_dq = combine_spectra(data_iso_dq, weights)
    data_z_dir_dq = combine_spectra(data_z_dir_dq, weights)
    data_y_dir_dq = combine_spectra(data_y_dir_dq, weights)
    data_x_dir_dq = combine_spectra(data_x_dir_dq, weights)

    # Just Quadrupole
    data_iso_jq = combine_spectra(data_iso_jq, weights)
    data_z_dir_jq = combine_spectra(data_z_dir_jq, weights)
    data_y_dir_jq = combine_spectra(data_y_dir_jq, weights)
    data_x_dir_jq = combine_spectra(data_x_dir_jq, weights)
   
    ### Z Propegation dir
    # linear
    ax[0][0].plot(data_iso_jd[0], data_iso_jd[1], label = "Iso", **plot_params_dict["iso"])
    ...
    ax[1][0].fill_between(data_x_lin_jd[0], np.zeros(len(linear_diff)), linear_diff, label = "X - Y")
    ax[1][0].legend(loc=2, handlelength = 1.0)
    ...
    ### Y Propegation dir
    # linear
    ax[0][1].plot(data_iso_jd[0], data_iso_jd[1], label = "Iso", **plot_params_dict["iso"])
    ...
    ax[1][1].fill_between(data_z_lin_jd[0], np.zeros(len(linear_diff)), linear_diff, label = "Z - X")
    ax[1][1].legend(loc=2, handlelength = 1.0)
    ...
    ### X Propegation dir
    # linear
    ax[0][2].plot(data_iso_jd[0], data_iso_jd[1], label = "Iso", **plot_params_dict["iso"])
    ...
    ax[1][2].fill_between(data_y_lin_jd[0], np.zeros(len(linear_diff)), linear_diff, label = "Y - Z")
    ax[1][2].legend(loc=2, handlelength = 1.0)
    ...
    title += " Polarized VtC-XES: " + ", ".join(list(set([re.sub(r'[0-9]', '', path.name.split("_")[-1]) for path in data_paths])))
    fig.suptitle(title, fontsize = 30, y = 0.95)
    fig.supylabel('Intensity (arb. units)', fontsize = 20, x = 0.05)
...
def make_LDOSPlots(data_paths, plot_params_dict):
    """
    Automatically create summary figure of LDOS from FEFF results.
    LDOS will be averaged over all directories in data_paths. This
    script assumes that the "isotropic" folder contains the ldos.dat
    files.
    """
    ...
    weights = [] # Used for weighting by symmetry multiplicty
    for data_path in data_paths:
        data_path = Path(data_path)
        weights.append(plot_params_dict["wyckoff_weights"][data_path.name.split("_")[-1]])
        
        fermienergy, sDOS_temp, pDOS_temp, dDOS_temp, fDOS_temp = extract_feff_ldos(data_path / "isotropic" / "ldos00.dat")

        X_SHIFT = -1 * fermienergy
        sDOS_temp[0] += X_SHIFT
        pDOS_temp[0] += X_SHIFT
        dDOS_temp[0] += X_SHIFT
        fDOS_temp[0] += X_SHIFT

        sDOS.append(sDOS_temp)
        pDOS.append(pDOS_temp)
        dDOS.append(dDOS_temp)
        fDOS.append(fDOS_temp)
        
    # Even though combine_spectra was intended for emission/absorption spec
    # it will work on any [[x],[y]] array
    sDOS = combine_spectra(sDOS, weights) 
    pDOS = combine_spectra(pDOS, weights)
    dDOS = combine_spectra(dDOS, weights)
    fDOS = combine_spectra(fDOS, weights)
    totalDOS = np.asarray([sDOS[0], sDOS[1] + pDOS[1] + dDOS[1] + fDOS[1]])

    plt.plot(sDOS[0], sDOS[1], label = "s-DOS", **plot_params_dict["sDOS"])
    plt.plot(pDOS[0], pDOS[1], label = "p-DOS", **plot_params_dict["pDOS"])
    plt.plot(dDOS[0], dDOS[1], label = "d-DOS", **plot_params_dict["dDOS"])
    plt.plot(fDOS[0], fDOS[1], label = "f-DOS", **plot_params_dict["fDOS"])
    plt.plot(totalDOS[0], totalDOS[1], label = "Total DOS", **plot_params_dict["totalDOS"])


    ### Aesthetics
    plt.axvline(0, dashes=[2,2], color="gray", linewidth=2) # Fermi Level
    wyckoffs_used = "[" + ", ".join([path.name.split("_")[-1] for path in data_paths]) + "]"
    plt.text(0.98, 0.65, wyckoffs_used, ha='right', va='top', transform=plt.gca().transAxes, fontsize=10)
    plt.legend(loc='upper right')

    # Axes Labels, Font Size, and Title
    title = CF.Convert_CompName_ToLaTeX(plot_params_dict["NAME"])
    plt.xlabel(r'Energy Relative to Fermi Level (eV)', fontsize = 15)
    title += " FEFF LDOS: " + ", ".join(list(set([re.sub(r'[0-9]', '', path.name.split("_")[-1]) for path in data_paths])))
    plt.title(title, fontsize = 20)
    plt.ylabel('LDOS (arb. units)', fontsize = 15)
