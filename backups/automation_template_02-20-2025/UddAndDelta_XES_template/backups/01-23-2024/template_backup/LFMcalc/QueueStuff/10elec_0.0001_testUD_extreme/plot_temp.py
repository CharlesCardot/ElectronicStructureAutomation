...
Udd = [2.0, 4.0, 6.0, 8.0]
Delta = ["{:.1f}".format(x) for x in np.arange(-40,20.1,4)]
...
for U in range(len(Udd)):
    ...
    ax[0].legend(loc=2, ncol=2, fontsize=8, title=legend_title)
    ax[0].set_ylabel("Intensity (arb. units)", fontsize=15)
    fig.suptitle("NAME", fontsize=20, y=0.95)
    plt.savefig("NAME_CT_param_explore_extreme.png", dpi=100)
    print("Successfully created", "NAME_CT_param_explore_extreme.png")
    #plt.show()
